...
public class appDAO_Impl implements appDAO {
    ...
    @Override
    @Transactional
    public void saveCourse(Course thecoCourse) {
        entityManager.persist(thecoCourse);
    }

    @Override
    public Course findCourseAndReviewByCourseID(int id) {
        TypedQuery<Course> querry = entityManager
                .createQuery("select c from Course c join fetch c.reviews where c.id=:data", Course.class);
        querry.setParameter("data", id);
        Course course = querry.getSingleResult();
        return course;
    }
    ...
    public Course findCourseAndStudentByID(int theID) {
        ...
        querry.setParameter("data", theID);
        Course course = querry.getSingleResult();
        return course;
    }
    ...
    public Student findStudentandCourseByStudentID(int id) {
        ...
        TypedQuery<Student> querry = entityManager
                .createQuery("select c from Student c join fetch c.courses where c.id=:data", Student.class);
        querry.setParameter("data", id);
        Student student = querry.getSingleResult();
        return student;
